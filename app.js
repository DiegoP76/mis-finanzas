...
async function registerBiometric() {
    try {
        ...
        await api('/api/webauthn/register/complete', { method: 'POST', body: reg });
        hasBiometric = true;
        return true;
    } catch (err) {
...
async function loginWithBiometric() {
    ...
    try {
        ...
        const data = await api('/api/webauthn/login/complete', { method: 'POST', body: { username, credential: assertion } });
...
async function removeBiometric() {
    try {
        await api('/api/webauthn/remove', { method: 'DELETE' });
        hasBiometric = false;
        updateBioBtn();
...
async function loadAllData() {
    const [txs, cats, pinData, bioData] = await Promise.all([
        api('/api/transactions'),
        api('/api/categories'),
        api('/api/pin'),
        api('/api/webauthn/has-credentials/' + encodeURIComponent(currentUser)),
    ]);
    transactions = txs;
    customUserCategories = cats;
    userPin = pinData.pin || '';
    hasBiometric = bioData.hasCredentials;
    rebuildCategoryMap();
}
...
function hidePin() { document.getElementById('pin-screen').classList.add('hidden'); }
...
function switchAuth(mode) {
    ...
    if (mode === 'login') {
        ...
        document.getElementById('bio-login-btn').style.display = '';
    } else {
        ...
        document.getElementById('bio-login-btn').style.display = 'none';
    }
}

async function checkBioOnInput() {
    updateBioLoginBtn();
}

function updateBioLoginBtn() {
    const btn = document.getElementById('bio-login-btn');
    const username = document.getElementById('auth-username').value.trim();
    if (authMode === 'login' && username.length >= 3 && window.PublicKeyCredential) {
        checkBiometric(username).then(has => {
...
async function handleAuth(e) {
    ...
    try {
        ...
        if (authMode === 'register' && window.PublicKeyCredential) {
            setTimeout(async () => {
                if (await registerBiometric()) showToast('Huella registrada');
            }, 500);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('auth-username').addEventListener('input', checkBioOnInput);
...
function renderTransactionHTML(tx) {
    ...
    return '<div class="transaction-item" data-id="' + tx.id + '">' +
        ...
        '<div class="transaction-info"><div class="transaction-desc">' + escapeHTML(tx.description) + '</div><div class="transaction-meta">' + formatDate(tx.date) + ' <span class="transaction-category">' + cat.label + '</span></div></div>' +
        '<span class="transaction-amount ' + tx.type + '">' + prefix + formatCurrency(tx.amount) + '</span>' +
        '<button class="delete-btn" onclick="deleteTransaction(' + tx.id + ')" title="Eliminar">&times;</button></div>';
}
...
function updateStatsSummary(txs) {
    ...
    document.getElementById('stats-daily').textContent = formatCurrency(Math.round(days > 0 ? expense / days : 0));
    document.getElementById('stats-daily').style.color = expense > 0 ? 'var(--expense)' : 'var(--text)';
    document.getElementById('stats-daily-income').textContent = formatCurrency(Math.round(days > 0 ? income / days : 0));
    document.getElementById('stats-daily-income').style.color = income > 0 ? '#00B894' : 'var(--text)';
...
function updateStatsDonutChart(txs) {
    ...
    statsDonutChart = new Chart(ctx, { type: 'doughnut', data: { labels: sorted.map(([id]) => { const c = CATEGORY_MAP[id]; return c ? c.label : id; }), datasets: [{ data: sorted.map(([, v]) => v), backgroundColor: sorted.map(([id]) => { const c = CATEGORY_MAP[id]; return c ? c.color : '#A0A4B8'; }), borderWidth: 3, borderColor: '#fff' }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'bottom', labels: { padding: 10, usePointStyle: true, pointStyle: 'circle', font: { size: 10 }, color: '#636e72' } } } } });
}
...
function updateStatsProjection(txs) {
    ...
    const remainingBudget = Math.round((monthIncome - monthExpense) + dailyRate * (daysInMonth - dayOfMonth));
    const projectedBalance = monthIncome - projectedExpense;
    card.style.display = 'block';
    body.innerHTML = '<div>Día <strong>' + dayOfMonth + '</strong> de <strong>' + daysInMonth + '</strong> &middot; <strong>' + (daysInMonth - dayOfMonth) + '</strong> días restantes</div>' +
        ...
        '<div>Balance proyectado: <strong class="' + (projectedBalance >= 0 ? 'highlight-green' : 'highlight-red') + '">' + formatCurrency(projectedBalance) + '</strong></div>' +
        '<div style="margin-top:6px;padding-top:8px;border-top:1px solid var(--border)">' +
        (remainingBudget >= 0 ? 'Te quedan <strong class="highlight-green">' + formatCurrency(remainingBudget) + '</strong> para el resto del mes' : 'Estás <strong class="highlight-red">' + formatCurrency(Math.abs(remainingBudget)) + '</strong> por encima de tu presupuesto') + '</div>';
}
...
function renderCatManager() {
    ...
    container.innerHTML = getAllCategories(catManagerType).map(c => {
        const isDefault = defaults.includes(c.id);
        const inUse = transactions.filter(tx => tx.type === catManagerType && tx.category === c.id).length;
        return '<div class="cat-manager-item"><span class="cat-info"><span class="cat-dot" style="background:' + c.color + '"></span> ' + c.icon + ' ' + c.label + (isDefault ? '<span style="font-size:10px;color:var(--text-muted);margin-left:4px">fija</span>' : '') + '</span>' +
            (isDefault ? '<button class="cat-remove" disabled title="No se puede eliminar"></button>' : '<button class="cat-remove" onclick="removeCat(\'' + catManagerType + '\',\'' + c.id + '\')" title="' + (inUse > 0 ? inUse + ' movimiento(s) asociado(s)' : 'Eliminar') + '">&times;</button>') + '</div>';
    }).join('');
}
...
function updateBioBtn() {
    const btn = document.querySelector('.bio-toggle-btn');
    if (btn) btn.style.opacity = hasBiometric ? '1' : '0.5';
}

function toggleBiometric() {
    if (hasBiometric) {
        if (confirm('¿Eliminar el acceso con huella?')) removeBiometric();
    } else {
        registerBiometric().then(ok => { if (ok) { updateBioBtn(); showToast('Huella registrada'); } });
    }
}

(function addSecurityBtns() {
    const headerTop = document.querySelector('.header-top');
    if (headerTop) {
        const bio = document.createElement('button');
        bio.className = 'bio-toggle-btn';
        bio.textContent = '🖐️';
        bio.style.cssText = 'background:none;border:none;font-size:16px;cursor:pointer;padding:0 0 0 6px';
        bio.title = 'Acceso con huella';
        bio.onclick = toggleBiometric;
        headerTop.appendChild(bio);

        const pin = document.createElement('button');
        pin.className = 'pin-toggle-btn';
        pin.textContent = '🔒';
        pin.style.cssText = 'background:none;border:none;font-size:16px;cursor:pointer;padding:0 0 0 6px';
        pin.title = 'Configurar PIN';
        pin.onclick = togglePin;
        headerTop.appendChild(pin);
